...
```bash
nextflow run catg-umag/nanotax --input ./samplesheet.csv --outdir ./results  -profile docker
```
...
```bash
nextflow run catg-umag/nanotax -profile docker -params-file params.yaml
```
...
```bash
nextflow pull catg-umag/nanotax
```
