...
workflow PIPELINE_INITIALISATION {
    ...
    Channel.fromList(samplesheetToList(params.input, "${projectDir}/assets/schema_input.json"))
        .map { meta, fastq, barcode, group, subgroup, subsubgroup ->
            group = group ?: false
            subgroup = subgroup ?: false
            subsubgroup = subsubgroup ?: false

            return [meta.id, meta + [barcode: barcode, group: group, subgroup: subgroup, subsubgroup: subsubgroup], fastq]
        }
        .groupTuple()
        .map { samplesheet ->
